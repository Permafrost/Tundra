...
// -----( CREATED: 2015-05-05 12:22:28 AEST
// -----( ON-HOST: -
...

{
        ...
        throws ServiceException
	{
		...
		try {
		    ...
		    IDataUtil.put(cursor, "$content", ObjectHelper.convert(FileHelper.readToBytes(file), encoding, mode));
		} catch(IOException ex) {
		    ExceptionHelper.raise(ex);
		} finally {
		    cursor.destroy();
        ...
        throws ServiceException
	{
		...
		try {
		    ...
		    file = FileHelper.writeFromStream(file, StreamHelper.normalize(content, encoding), mode == null || mode.equalsIgnoreCase("append"));
	    ...
	    if (file != null) {
	        ...
	        try {
	            if (mode == null || mode.equals("read")) {
	                stream = StreamHelper.normalize(new java.io.FileInputStream(file));
	            } else {
	                if (!FileHelper.exists(file)) FileHelper.create(file);
	                stream = StreamHelper.normalize(new java.io.FileOutputStream(file, mode.equalsIgnoreCase("append")));
	            }
